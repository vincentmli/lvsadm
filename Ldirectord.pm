package Config::Ldirectord;

# $Id: Ldirectord.pm,v 3.55 2005/02/10 18:57:16 sherzodr Exp $
...
sub parse_ini_file {
  ...
  while ( defined($line=<$fh>) ) {
    ...
    if ( $line =~ /^\s*([^=]*\w)\s*=\s*(.*)\s*$/ ) {
	if ( exists $data{$bn}->{lcase($1)} ) {
		push @{$data{$bn}->{lcase($1)}}, parse_line(READ_DELIM, 0, $2); 
		
	} else {
	$data{$bn}->{lcase($1)}=[parse_line(READ_DELIM, 0, $2)];
	}
	next;
    }
...
sub as_string {
    ...
    my $STRING = undef;
    if ( $syntax eq 'ini' ) {
        $STRING .= "# Config::Ldirectord $VERSION\n";
        $STRING .= "# $currtime\n\n";
...
Config::Ldirectord - simple configuration file class

=head1 SYNOPSIS

  use Config::Ldirectord;

  # --- Ldirectord usage. Loads the config. file into a hash:
  Config::Ldirectord->import_from('app.ini', \%Config);


  # --- OO interface:
  $cfg = new Config::Ldirectord('app.ini');
  ...
  # --- tie() interface
  tie %Config, "Config::Ldirectord", 'app.ini';


=head1 ABSTRACT

Reading and writing configuration files is one of the most frequent
tasks of any software design. Config::Ldirectord is the library that helps
you with it.

Config::Ldirectord is a class representing configuration file object. 
...
an external file. That's where Config::Ldirectord comes into play, making it
...
Ldirectord syntax is what you need for most of your projects. These
...
While working with such files, Config::Ldirectord assigns them to a default block, 
...
and a reference to an existing (possibly empty) hash:
  ...
  Config::Ldirectord->import_from('myconf.cfg', \%Config);
...
much more Config::Ldirectord offers.
...
interface:
  ...
  $cfg = new Config::Ldirectord('app.cfg');
...
Config::Ldirectord also supports vars() method, which, depending on the context
...
substituted with underscore '_':
  ...
  Config::Ldirectord->new('app.cfg')->import_names();
...
name space as the first argument. It also prevents potential name collisions:
  ...
  Config::Ldirectord->new('app.cfg')->import_names('CFG');
...
seem longer than necessary. That's why Config::Ldirectord supports import_from() - class method,
which is called with the name of the configuration file. It will call import_names() for you:
  
  Config::Ldirectord->import_from('app.cfg');
...
of the configuration file.
  ...
  Config::Ldirectord->import_from('app.cfg', 'CFG');

  # import into %Config hash:
  Config::Ldirectord->import_from('app.cfg', \%Config);

The above line imports all the values to 'CFG' name space. import_from() returns
underlying Config::Ldirectord object (which you may not even need anymore):

  $cfg = Config::Ldirectord->import_from('app.cfg', \my %Config);
  $cfg->write('app.cfg.bak');

=head2 UPDATING THE VALUES

Configuration values, once read into Config::Ldirectord, can be updated from within
...
is terminated, all the configuration values are written back to its file:
  ...
  $cfg = new Config::Ldirectord('aff.cfg');
  $cfg->autosave(1);
...
scratch using Config::Ldirectord, simply create an empty configuration file object
...
When you're done, call write() method with the name of the configuration file:
  ...
  $cfg = new Config::Ldirectord(syntax=>'ini');
...
That's why Config::Ldirectord supports this fancy feature as well. Simply separate your values
with a comma:
...
To access the method provided in OO syntax, you need to get underlying Config::Ldirectord
...
But it is possible to force Config::Ldirectord to ignore cases all together by enabling
C<-lc> switch while loading the library:

  use Config::Ldirectord ('-lc');
...
enclosed in double quotes. Well, says them! Although Config::Ldirectord supports parsing
...
Config::Ldirectord doesn't believe in dying that easily (unless you insult it using wrong syntax).
...
undef otherwise:
  ...
  $cfg = new Config::Ldirectord('app.cfg') or die Config::Ldirectord->error();

  # import_from() calls read(), so it can fail:
  Config::Ldirectord->import_from('app.cfg', \%Config) or die Config::Ldirectord->error();

  # write() may fail:
  $cfg->write() or die $cfg->error();

  # tie() may fail, since it calls new() with a filename
  tie %Config, "Config::Ldirectord", 'app.cfg' or die Config::Ldirectord->error();
...
- constructor. Optionally accepts several arguments. Returns Config::Ldirectord object.
...
if used without arguments. In 'autosave' mode Config::Ldirectord writes all the changes
...
- for debugging only. Dumps the whole Config::Ldirectord object using Data::Dumper.
...
L<Config::General>, L<Config::Ldirectord>, L<Config::Tiny>
...
sub import_names {
  ...
  if ( $namespace eq 'Config::Ldirectord') {
    croak "You cannot import into 'Config::Ldirectord' package";
  }
  my %vars = $self->vars();
  no strict 'refs';
...
sub TIEHASH {
  ...
  unless ( defined $file ) {
    croak "Usage: tie \%config, 'Config::Ldirectord', \$filename";
  }  
